    ...
    public static async ValueTask<object?> EvaluateAsync(string functionName, AsyncExpression[] arguments, AsyncExpressionContext context, ExpressionLocation location, CancellationToken cancellationToken = default)
    {
        ...
        if (functionName.Equals("ifs", comparison))
        {
            ...
            foreach (var argument in arguments.Where((_, i) => i % 2 == 0))
            {
                ...
                if (tf)
                    return await arguments[index + 1].EvaluateAsync(cancellationToken).ConfigureAwait(false);
            }
        ...
        if (functionName.Equals("if", comparison))
        {
            ...
            return cond ? await arguments[1].EvaluateAsync(cancellationToken).ConfigureAwait(false) : await arguments[2].EvaluateAsync(cancellationToken).ConfigureAwait(false);
        }
